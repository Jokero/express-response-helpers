...
function asyncValidate(value, config, path, originalObject) {
    ...
    var resolvedValidators = resultValue(config, value, path, originalObject);
    for (validatorName in resolvedValidators) {
        resolvedValidators[validatorName] = resultValue(resolvedValidators[validatorName], value, path, originalObject);
    }

    for (validatorName in resolvedValidators) {
        validator        = validateJs.validators[validatorName];
        validationResult = validator.call(validator, value, resolvedValidators[validatorName], originalObject);
