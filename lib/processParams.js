 ...
 * @returns {Object}
 */
function setDefaultValue(object, config) {
    var fieldConfig;
    ...
    for (var field in config) {
        ...
        if (fieldConfig instanceof Object) {
            ...
            if (fieldConfig.properties instanceof Object) {
                setDefaultValue(object[field], fieldConfig.properties);
            }
        }
    }
...
function validateObject(object, config, path, originalObject) {
    var fieldConfig, fieldValue, fieldPath;
...
function asyncValidate(value, config, path, originalObject) {
    ...
    var resolvedValidators = validateJs.result(config, value, path, originalObject);
    for (var name in resolvedValidators) {
        resolvedValidators[name] = validateJs.result(resolvedValidators[name], value, path, originalObject);
    }

    for (var name in resolvedValidators) {
        validator        = validateJs.validators[name];
        validationResult = validator.call(validator, value, resolvedValidators[name], originalObject);
...
module.exports = function(object, config) {
    ...
    return Promise.resolve(transform(object, config))
        .then(function(object) {
            return setDefaultValue(object, config);
        })
        .then(function(object) {
